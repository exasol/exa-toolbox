...
class ReleaseLinkExtractor:
    ...
    def get_link_by_release_name(self, release_name):
        """
        ...
        result_link = self.__find_link(list_of_available_releases, release_name)
        if result_link is not None:
            return result_link
        else:
            raise ValueError(
                'Release with the name ' + release_name + ' was not found. Please check the name or select another release')
