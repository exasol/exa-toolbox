...
function create_col_str(schema, table)
  res = query([[with notnulls_constraints as
  (
...
function create_dist_key_str(src_schema, src_table, trgt_schema, trgt_table)
  res = query([[select 'ALTER TABLE '||:trgt_obj||' DISTRIBUTE BY '||GROUP_CONCAT('"'||column_name||'"' ORDER BY column_ordinal_position)||';' as DDL_TEXT
...
function create_part_key_str(src_schema, src_table, trgt_schema, trgt_table)
  res = query([[select 'ALTER TABLE '||:trgt_obj||' PARTITION BY '||GROUP_CONCAT('"'||column_name||'"' ORDER BY COLUMN_PARTITION_KEY_ORDINAL_POSITION)||';' as DDL_TEXT
from exa_all_columns
where column_schema = :sch
  and column_table  = :tab
...
-- creates string for ZONE MAPS
-- by design partioning creates zone maps on partition columns
-- so we only need to handle situations when it's dropped for a partition column or added on a non-partition column
-- returns bool(at least 1 column to change zone maps), array with ddl(if there are columns to change zone maps)
function create_zone_maps_str(src_schema, src_table, trgt_schema, trgt_table)
  res = query([[select case when COLUMN_IS_ZONEMAPPED then 'ENFORCE' else 'DROP' end || ' ZONEMAP ON '||:trgt_obj||'("'||column_name||'");' as DDL_TEXT
from exa_all_columns
where column_schema = :sch
  and column_table  = :tab
...
function create_table_comments(src_schema, src_table, trgt_schema, trgt_table)
  res = query([[select 'COMMENT ON TABLE '||:trgt_obj||' is '''||TABLE_COMMENT||'''' as DDL_TEXT
from EXA_ALL_TABLES
where table_schema = :sch
  and table_name   = :tab                  
]], {sch = src_schema, tab = src_table, trgt_obj=join('.', quote(trgt_schema), quote(trgt_table))})
  res2 = query([[select GROUP_CONCAT('"'||column_name||'" is '''||column_comment||'''') as DDL_TEXT
from EXA_ALL_COLUMNS
where column_schema = :sch
  and column_table  = :tab
...
-- ##### partition key
partk_exists, partk_str = create_part_key_str(src_schema, src_table, trgt_schema, trgt_table)
if (partk_exists) then
  ddl_str = ddl_str..'\n'..partk_str  
end

-- ##### zone maps
if (version_major >= 8) then
	zm_exists, zm_strs = create_zone_maps_str(src_schema, src_table, trgt_schema, trgt_table)
	if (zm_exists) then 
	  for i=1,#zm_strs do
	    ddl_str = ddl_str..'\n'..zm_strs[i][1]    
	  end
